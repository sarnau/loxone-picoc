...
int IsTypeToken(struct ParseState * Parser, enum LexToken t, struct Value * LexValue)
{
    ...
    if (t == TokenIdentifier) /* see TypeParseFront, case TokenIdentifier and ParseTypedef */
    {
        ...
        if (VariableDefined(Parser->pc, (char *) LexValue->Val->Pointer))
        {
            VariableGet(Parser->pc, Parser, (char *) LexValue->Val->Pointer, &VarValue);
...
int64_t ExpressionCoerceLong(struct Value *Val)
{
    switch (Val->Typ->Base)
    {
        ...
        case TypeLong:            return (int64_t)Val->Val->LongInteger;
        ...
        case TypePointer:         return (uintptr_t)Val->Val->Pointer;
#ifndef NO_FP
...
int64_t ExpressionAssignInt(struct ParseState *Parser, struct Value *DestValue, int64_t FromInt, int After)
{
    ...
    if (After)
        Result = ExpressionCoerceLong(DestValue);
    else
        Result = FromInt;
...
void ExpressionPushLong(struct ParseState *Parser, struct ExpressionStack **StackTop, int64_t LongValue)
{
    struct Value *ValueLoc = VariableAllocValueFromType(Parser->pc, Parser, &Parser->pc->LongType, FALSE, NULL, FALSE);
    ValueLoc->Val->LongInteger = LongValue;
    ExpressionStackPushValueNode(Parser, StackTop, ValueLoc);
}
...
void ExpressionAssignToPointer(struct ParseState *Parser, struct Value *ToValue, struct Value *FromValue, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    else if (IS_NUMERIC_COERCIBLE(FromValue) && ExpressionCoerceLong(FromValue) == 0)
    {
    ...
    else if (AllowPointerCoercion && IS_NUMERIC_COERCIBLE(FromValue))
    {
        ...
        ToValue->Val->Pointer = (void *)(uintptr_t)ExpressionCoerceUnsignedLong(FromValue);
    }
...
void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    switch (DestValue->Typ->Base)
    {
        case TypeInt:           DestValue->Val->Integer = (int)ExpressionCoerceLong(SourceValue); break;
        case TypeShort:         DestValue->Val->ShortInteger = (short)ExpressionCoerceLong(SourceValue); break;
        case TypeChar:          DestValue->Val->Character = (char)ExpressionCoerceLong(SourceValue); break;
        case TypeLong:          DestValue->Val->LongInteger = ExpressionCoerceLong(SourceValue); break;
        case TypeUnsignedInt:   DestValue->Val->UnsignedInteger = (unsigned int) ExpressionCoerceUnsignedLong(SourceValue); break;
        case TypeUnsignedShort: DestValue->Val->UnsignedShortInteger = (unsigned short)ExpressionCoerceUnsignedLong(SourceValue); break;
        case TypeUnsignedLong:  DestValue->Val->UnsignedLongInteger = ExpressionCoerceUnsignedLong(SourceValue); break;
        case TypeUnsignedChar:  DestValue->Val->UnsignedCharacter = (unsigned char)ExpressionCoerceUnsignedLong(SourceValue); break;
...
void ExpressionQuestionMarkOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *BottomValue, struct Value *TopValue)
{
    ...
    if (ExpressionCoerceLong(TopValue))
    {
    ...
    switch (Op)
    {
        ...
        case TokenSizeof:
            ...
            if (TopValue->Typ == &Parser->pc->TypeType)
                ExpressionPushLong(Parser, StackTop, TypeSize(TopValue->Val->Typ, TopValue->Val->Typ->ArraySize, TRUE));
            else
                ExpressionPushLong(Parser, StackTop, TypeSize(TopValue->Typ, TopValue->Typ->ArraySize, TRUE));
            break;
        ...
        default:
            ...
            if (IS_NUMERIC_COERCIBLE(TopValue))
            {
                ...
                int64_t ResultInt = 0;
                int64_t TopInt = ExpressionCoerceLong(TopValue);
                switch (Op)
                {
...
void ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
    ...
    { 
        ...
        ArrayIndex = (int) ExpressionCoerceLong(TopValue);
              ...
              (IS_NUMERIC_COERCIBLE(TopValue) && BottomValue->Typ == &Parser->pc->FPType) )
    {
        ...
        double ResultFP = 0.0;
        double TopFP = (TopValue->Typ == &Parser->pc->FPType) ? TopValue->Val->FP : (double)ExpressionCoerceLong(TopValue);
        double BottomFP = (BottomValue->Typ == &Parser->pc->FPType) ? BottomValue->Val->FP : (double)ExpressionCoerceLong(BottomValue);
    ...
    { 
        /* integer operation */
        int64_t TopInt = ExpressionCoerceLong(TopValue);
        int64_t BottomInt = ExpressionCoerceLong(BottomValue);
        switch (Op)
        {
    ...
    else if (BottomValue->Typ->Base == TypePointer && IS_NUMERIC_COERCIBLE(TopValue))
    {
        ...
        if (Op == TokenEqual || Op == TokenNotEqual)
        {
            ...
            if (Op == TokenEqual)
                ExpressionPushLong(Parser, StackTop, BottomValue->Val->Pointer == NULL);
            else
                ExpressionPushLong(Parser, StackTop, BottomValue->Val->Pointer != NULL);
        }
    ...
    else if (BottomValue->Typ->Base == TypePointer && TopValue->Typ->Base == TypePointer && Op != TokenAssign)
    {
        ...
        switch (Op)
        {
            case TokenEqual:                ExpressionPushLong(Parser, StackTop, BottomLoc == TopLoc); break;
            case TokenNotEqual:             ExpressionPushLong(Parser, StackTop, BottomLoc != TopLoc); break;
            case TokenMinus:                ExpressionPushLong(Parser, StackTop, (int) (BottomLoc - TopLoc)); break;
            default:                        ProgramFail(Parser, "invalid operation"); break;
        }
    }
                ...
                case OrderPrefix:
                    ...
                    *StackTop = TopOperatorNode->Next;
                    ...
                    else
                    {
                        ...
                        ExpressionPushLong(Parser, StackTop, 0);
                    }
                    break;
...
void ExpressionStackPushOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum OperatorOrder Order, enum LexToken Token, int Precedence)
{
    struct ExpressionStack *StackNode = (struct ExpressionStack *) VariableAlloc(Parser->pc, Parser, sizeof(struct ExpressionStack), FALSE);
    StackNode->Next = *StackTop;
    StackNode->Order = (unsigned char) Order;
    StackNode->Op = Token;
...
void ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
    ...
    { 
        ...
        if (Token == TokenArrow)
            DerefDataLoc = (char *) VariableDereferencePointer(ParamVal, &StructVal, NULL, &StructType, NULL);
            ...
            { 
                ...
                { 
                    ...
                    { 
                        ...
                        {
                            int64_t LHSInt = ExpressionCoerceLong(StackTop->Val);
...
int64_t ExpressionParseInt(struct ParseState *Parser)
{
    ...
    { 
        ...
        Result = ExpressionCoerceLong(Val);
