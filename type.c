...
void TypeInit(Picoc *pc)
{
    ...
    struct LongAlign { char x; int64_t y; } la;
#ifndef NO_FP
    ...
    TypeAddBaseType(pc, &pc->LongType, TypeLong, sizeof(int64_t), (int) ((char *)&la.y - &la.x));
    ...
    TypeAddBaseType(pc, &pc->UnsignedLongType, TypeUnsignedLong, sizeof(uint64_t), (int) ((char *)&la.y - &la.x));
